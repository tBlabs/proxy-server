...
export class Action2 extends React.Component<{}, IState>
{
    constructor(props: any)
    {
        super(props);
    ...
    render()
    {
        ...
        return (
            <div>
                <RadioGroup>
                    <FormControlLabel value="StoreInInternalMemory" control={ <Switch /> } disabled label="INTERNAL MEMORY" />
                    <FormControlLabel control={ <Switch /> } label="EXTERNAL MEMORY" onChange={ async (e: any) => device.Config.StoreInExMem = e.target.checked ) } />
                    <FormControlLabel control={ <Switch /> } label="CLOUD" onChange={ async (e: any) => await this.Update({ StoreInCloud: e.target.checked }) } />
                </RadioGroup>
            </div>
        );
